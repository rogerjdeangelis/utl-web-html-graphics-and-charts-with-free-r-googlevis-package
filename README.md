# utl-web-html-graphics-and-charts-with-free-r-googlevis-package
web html graphics and charts with free r googlevis package
    %let pgm=utl-web-html-graphics-and-charts-with-free-r-googlevis-package;

    web html graphics and charts with free r googlevis package

      Three Examples

           1. Organization Chart
           2. us map
           3. line plot
           4. blank ascii map of us
           
    You need to download to view (i could not get any of the github options to work htmlpreview, ctl=shift-V, varous hackers)

    https://tinyurl.com/bdcp5wx6  (line chart)
    https://github.com/rogerjdeangelis/utl-web-html-graphics-and-charts-with-free-r-googlevis-package/blob/main/googlinecht.html

    https://tinyurl.com/35akpty7 (org cjart)
    https://github.com/rogerjdeangelis/utl-web-html-graphics-and-charts-with-free-r-googlevis-package/blob/main/googorgcht.html

    https://tinyurl.com/4ytxedc9 (map)
    https://github.com/rogerjdeangelis/utl-web-html-graphics-and-charts-with-free-r-googlevis-package/blob/main/googusmap.html

    github
    https://tinyurl.com/mve958jj
    https://github.com/rogerjdeangelis/utl-web-html-graphics-and-charts-with-free-r-googlevis-package

    github
    https://tinyurl.com/mve958jj
    https://github.com/rogerjdeangelis/utl-web-html-graphics-and-charts-with-free-r-googlevis-package

    Related repos

    https://github.com/rogerjdeangelis/utl-dygraph-javascript-library-from-MIT
    https://github.com/rogerjdeangelis/utl_javascript_and_classic_map_graphics_with_mouseovers_and_multiple_drilldowns
    https://github.com/rogerjdeangelis/utl_javascript_dygraph_graphics_multipanel_time_series
    https://github.com/rogerjdeangelis/utl-excel-hiperlinks-click-on-your-favorite-baseball-player-and-a-google-search-will-pop-up
    https://github.com/rogerjdeangelis/utl-google-trends-is-the-seach-word-increasing-in-popularity
    https://github.com/rogerjdeangelis/utl-google-weekly-trends-for_2019-on-my-last-name-r-gtrendsr-package
    https://github.com/rogerjdeangelis/utl-universal-language-translator-using-python-package-googletrans
    https://github.com/rogerjdeangelis/utl_driving_distance_from_city_to_city_using_google_maps
    https://github.com/rogerjdeangelis/utl_google_and_SAS_maps
    https://github.com/rogerjdeangelis/utl_google_map_of_USA_with_long_and_lat_and_earnings
    https://github.com/rogerjdeangelis/utl_google_map_of_earnings_for_canada_cities
    https://github.com/rogerjdeangelis/utl_spelling-corrected-using-google-search
    https://github.com/rogerjdeangelis/utl_using_google_tesseract_and_sas_to_convert_bmp_formatted_picture_christmas_trees_to_ascii_txt

    /**************************************************************************************************************************/
    /*                                                                                                                        */
    /* 1. ORGANIZATION CHART                                                                                                  */
    /* =====================                                                                                                  */
    /*                                                +---------+                                                             */
    /*                                                |         |                                                             */
    /*               +--------------------------------| America |-------------------------------+                             */
    /*               |                                |         |                               |                             */
    /*               |                                +---------+                               |                             */
    /*               |                                     |                                    |                             */
    /*          +---------+                           +---------+                           +---------+                       */
    /*          |         |                           |         |                           |         |                       */
    /*          | America |                           | Europe  |                           |   Asia  |                       */
    /*          |         |                           |         |                           |         |                       */
    /*          +---------+                           +---------+                           +---------+                       */
    /*               |                                     |                                    |                             */
    /*       +-------------+                 +---------------------------+                +------------ +                     */
    /*       |             |                 |                           |                |             |                     */
    /*   +---------+   +---------+      +---------+   +---------+   +---------+      +---------+   +---------+                */
    /*   |         |    |         |      |         |   |         |   |         |      |         |   |         |               */
    /*   | Mexico  |   |   USA   |      | France  |   | Sweden  |   | Germany |      | China   |   |  Japan  |                */
    /*   |         |   |         |      |         |   |         |   |         |      |         |   |         |                */
    /*   +---------+   +---------+      +---------+   +---------+   +---------+      +---------+   +---------+                */
    /*                                                                                                                        */
    /*                                                                                                                        */
    /* 2.US MAP                                                                                                               */
    /* ========                                                                                                               */
    /*                  ________________________                            / \                                               */
    /*                   |    ||         |      |------ _  _           ____(  _)                                              */
    /*                  |     |(         |      |    /_/ \// _        / | || /                                                */
    /*                  |_____| \        |------|   /  --\/ / \    __/  | /|(                                                 */
    /*                  |     |  \_------|      |   \_   | (  (   /     |/_|/                                                 */
    /*                  |     |    |     |______|_____\ _| |   \_/ ___ _|_|/                                                  */
    /*                  (_____|____|     |       \     \ \_)------/   ) )                                                     */
    /*                  |   |    | |_____|__      )____/  |  |    |___)/                                                      */
    /*                  (   |    |    |     |_____|    \  |  /---/\__  \                                                      */
    /*                  )   |    |    |     |      \    \ )_/    \ / \_/                                                      */
    /*                  \   \    |    |     |       |    ||______//____|                                                      */
    /*                   |   \   |____|_____|_______|----|       /      \                                                     */
    /*                   )    \ 1/1111|2222|11111111|3333/______/_____  /                                                     */
    /*                   \     \|11111|2222|11|11111|3333111|222/1\111\/                                                      */
    /*                    \     \11111|2222|11|11111|---/111|222\11\11/                                                       */
    /*                     \_   /11111|2222|33333111)111)111|2222111\/                                                        */
    /*                       \__\_____|_----13333333\111|-|_|_---___/                                                         */
    /*                                   \33333333333)1111|   \3\222\                                                         */
    /*                                    \3/\333333/   11        )22\                                                        */
    /*                                        \3333/    --        \22(                                                        */
    /*                                         \33(                |22|                                                       */
    /*                Literacy Rates Us South   \33\                \2|                                                       */
    /*                                                               -                                                        */
    /* 3 LINE PLOT                                                                                                            */
    /* ===========                                                                                                            */
    /*                                                                                                                        */
    /*                                                                                                                        */
    /*                                  Car Type                                                                              */
    /*                                                                                                                        */
    /*                 Hybrid     SUV     Sedan   Sports   Truck   Wagon                                                      */
    /*                 ---+--------+--------+--------+-------+--------+---                                                    */
    /*                 |                                                 |                                                    */
    /*              60 +  X                                              +  60                                                */
    /*                 |   \         MPG by Car Type                     |                                                    */
    /*                 |    \                                            |                                                    */
    /*                 |     \                                           |     M                                              */
    /*           M  40 +      \                                          +  40 P                                              */
    /*           P     |       \                                         |     G                                              */
    /*           G     |        \         / X                            |                                                    */
    /*                 |         \      /    \                    ._-X   |                                                    */
    /*              20 +          \   /       \             /X  ./       +  20                                                */
    /*                 |           X/          \           /  \/      \  |                                                    */
    /*                 |                        \        /               |                                                    */
    /*                 |                         \     /                 |                                                    */
    /*               0 +                          \   /                  +   0                                                */
    /*                 |                            \X                   |                                                    */
    /*                 ---+--------+--------+--------+-------+-------+----                                                    */
    /*                 Hybrid     SUV     Sedan   Sports   Truck   Wagon                                                      */
    /*                                                                                                                        */
    /*                                 Car Type                                                                               */
    /*                                                                                                                        */
    /**************************************************************************************************************************/

    /*                               _          _   _                   _                _
    / |   ___  _ __ __ _  __ _ _ __ (_)______ _| |_(_) ___  _ __    ___| |__   __ _ _ __| |_
    | |  / _ \| `__/ _` |/ _` | `_ \| |_  / _` | __| |/ _ \| `_ \  / __| `_ \ / _` | `__| __|
    | | | (_) | | | (_| | (_| | | | | |/ / (_| | |_| | (_) | | | || (__| | | | (_| | |  | |_
    |_|  \___/|_|  \__, |\__,_|_| |_|_/___\__,_|\__|_|\___/|_| |_| \___|_| |_|\__,_|_|   \__|
     _              |___/_
    (_)_ __  _ __  _   _| |_
    | | `_ \| `_ \| | | | __|
    | | | | | |_) | |_| | |_
    |_|_| |_| .__/ \__,_|\__|
            |_|
    */

    options validvarname=v7;
    libname sd1 "d:/sd1";
    data sd1.havOrg;
    retain Region Parent Val Fac;
    input;
     Region = scan(_infile_,1);
     Val    = scan(_infile_,2)*1;
     Fac    = scan(_infile_,3)*1;
     Parent = scan(_infile_,4);
    cards4;
     Global   10   2       X
    America    2   4  Global
     Europe   99  11  Global
       Asia   10   8  Global
     France   71   2  Europe
     Sweden   89   3  Europe
    Germany   58  10  Europe
     Mexico    2   9 America
        USA   38  11 America
      China    5   1    Asia
      Japan   48  11    Asia
    ;;;;
    run;quit;

    /*
     _ __  _ __ ___   ___ ___  ___ ___
    | `_ \| `__/ _ \ / __/ _ \/ __/ __|
    | |_) | | | (_) | (_|  __/\__ \__ \
    | .__/|_|  \___/ \___\___||___/___/
    |_|
    */


    %utlfkil(d:/htm/googorgcht.html);
    %utl_submit_r64('
    library(haven);
    library(googleVis);
    Regions<-read_sas("d:/sd1/havOrg.sas7bdat");
    Regions[1,2]<-NA;
    Regions$Parent<-as.factor(Regions$Parent);
    Regions$Region<-as.factor(Regions$Region);
    Regions$Val<-as.integer(Regions$Val);
    Regions$Fac<-as.integer(Regions$Fac);
    Regions<-data.frame(Regions);
    Regions;
    str(Regions);
    Org <- gvisOrgChart(Regions,
     options=list(width=600, height=250,
     size="large", allowCollapse=TRUE));
    print(Org, file="d:/htm/googorgcht.html");
    ');

    /*           _               _
      ___  _   _| |_ _ __  _   _| |_
     / _ \| | | | __| `_ \| | | | __|
    | (_) | |_| | |_| |_) | |_| | |_
     \___/ \__,_|\__| .__/ \__,_|\__|
                    |_|
    */

    /**************************************************************************************************************************/
    /*                                                                                                                        */
    /* 1. ORGANIZATION CHART                                                                                                  */
    /*                                                                                                                        */
    /*                                                +---------+                                                             */
    /*                                                |         |                                                             */
    /*               +--------------------------------| America |-------------------------------+                             */
    /*               |                                |         |                               |                             */
    /*               |                                +---------+                               |                             */
    /*               |                                     |                                    |                             */
    /*          +---------+                           +---------+                           +---------+                       */
    /*          |         |                           |         |                           |         |                       */
    /*          | America |                           | Europe  |                           |   Asia  |                       */
    /*          |         |                           |         |                           |         |                       */
    /*          +---------+                           +---------+                           +---------+                       */
    /*               |                                     |                                    |                             */
    /*       +-------------+                 +---------------------------+                +------------ +                     */
    /*       |             |                 |                           |                |             |                     */
    /*   +---------+   +---------+      +---------+   +---------+   +---------+      +---------+   +---------+                */
    /*   |         |    |         |      |         |   |         |   |         |      |         |   |         |                */
    /*   | Mexico  |   |   USA   |      | France  |   | Sweden  |   | Germany |      | China   |   |  Japan  |                */
    /*   |         |   |         |      |         |   |         |   |         |      |         |   |         |                */
    /*   +---------+   +---------+      +---------+   +---------+   +---------+      +---------+   +---------+                */
    /*                                                                                                                        */
    /**************************************************************************************************************************/


    /*___
    |___ \   _   _ ___   _ __ ___   __ _ _ __
      __) | | | | / __| | `_ ` _ \ / _` | `_ \
     / __/  | |_| \__ \ | | | | | | (_| | |_) |
    |_____|  \__,_|___/ |_| |_| |_|\__,_| .__/
     _                   _              |_|
    (_)_ __  _ __  _   _| |_
    | | `_ \| `_ \| | | | __|
    | | | | | |_) | |_| | |_
    |_|_| |_| .__/ \__,_|\__|
            |_|
    */

    options validvarname=v7;
    data tmp.states;
     input state_name $16. Illiteracy 5.1 +1 @@;
    cards4;
    Alabama           2.1 Alaska            1.5 Arizona           1.8
    Arkansas          1.9 California        1.1 Colorado          0.7
    Connecticut       1.1 Delaware          0.9 Florida           1.3
    Georgia           2.0 Hawaii            1.9 Idaho             0.6
    Illinois          0.9 Indiana           0.7 Iowa              0.5
    Kansas            0.6 Kentucky          1.6 Louisiana         2.8
    Maine             0.7 Maryland          0.9 Massachusetts     1.1
    Michigan          0.9 Minnesota         0.6 Mississippi       2.4
    Missouri          0.8 Montana           0.6 Nebraska          0.6
    Nevada            0.5 New Hampshire     0.7 New Jersey        1.1
    New Mexico        2.2 New York          1.4 North Carolina    1.8
    North Dakota      0.8 Ohio              0.8 Oklahoma          1.1
    Oregon            0.6 Pennsylvania      1.0 Rhode Island      1.3
    South Carolina    2.3 South Dakota      0.5 Tennessee         1.7
    Texas             2.2 Utah              0.6 Vermont           0.6
    Virginia          1.4 Washington        0.6 West Virginia     1.4
    Wisconsin         0.7 Wyoming           0.6
    ;;;;
    run;quit;

    /*
     _ __  _ __ ___   ___ ___  ___ ___
    | `_ \| `__/ _ \ / __/ _ \/ __/ __|
    | |_) | | | (_) | (_|  __/\__ \__ \
    | .__/|_|  \___/ \___\___||___/___/
    |_|
    */

    %utlfkil(d:/htm/googusmap.html);

    %utl_submit_r64('
    library(haven);
    library(googleVis);
    states<-read_sas("c:/temp/states.sas7bdat");
    states;
    mp <- gvisGeoChart(
         states
       ,"state_name"
       ,"Illiteracy"
       ,options=list(region="US"
       ,displayMode="regions"
       ,resolution="provinces"
       ,width=600
       ,height=400));
    print(mp, file="d:/htm/googusmap.html");
    ');

    /*           _               _
      ___  _   _| |_ _ __  _   _| |_
     / _ \| | | | __| `_ \| | | | __|
    | (_) | |_| | |_| |_) | |_| | |_
     \___/ \__,_|\__| .__/ \__,_|\__|
                    |_|
    */


    /**************************************************************************************************************************/
    /*                                                                                                                        */
    /* 2.US MAP Literacy Rates US South                                                                                       */
    /*                                                                                                                        */
    /*                        ________________________                            / \                                         */
    /*                         |    ||         |      |------ _  _           ____(  _)                                        */
    /*                        |     |(         |      |    /_/ \// _        / | || /                                          */
    /*                        |_____| \        |------|   /  --\/ / \    __/  | /|(                                           */
    /*                        |     |  \_------|      |   \_   | (  (   /     |/_|/                                           */
    /*                        |     |    |     |______|_____\ _| |   \_/ ___ _|_|/                                            */
    /*                        (_____|____|     |       \     \ \_)------/   ) )                                               */
    /*                        |   |    | |_____|__      )____/  |  |    |___)/                                                */
    /*                        (   |    |    |     |_____|    \  |  /---/\__  \                                                */
    /*                        )   |    |    |     |      \    \ )_/    \ / \_/                                                */
    /*                        \   \    |    |     |       |    ||______//____|                                                */
    /*                         |   \   |____|_____|_______|----|       /      \                                               */
    /*                         )    \ 1/1111|2222|11111111|3333/______/_____  /                                               */
    /*                         \     \|11111|2222|11|11111|3333111|222/1\111\/                                                */
    /*                          \     \11111|2222|11|11111|---/111|222\11\11/                                                 */
    /*                           \_   /11111|2222|33333111)111)111|2222111\/                                                  */
    /*                             \__\_____|_----13333333\111|-|_|_---___/                                                   */
    /*                                         \33333333333)1111|   \3\222\                                                   */
    /*                                          \3/\333333/   11        )22\                                                  */
    /*                                              \3333/    --        \22(                                                  */
    /*                                               \33(                |22|                                                 */
    /*                      Literacy Rates US South   \33\                \2|                                                 */
    /*                                                                     -                                                  */
    /**************************************************************************************************************************/

    /*____   _ _                    _       _
    |___ /  | (_)_ __   ___   _ __ | | ___ | |_
      |_ \  | | | `_ \ / _ \ | `_ \| |/ _ \| __|
     ___) | | | | | | |  __/ | |_) | | (_) | |_
    |____/  |_|_|_| |_|\___| | .__/|_|\___/ \__|
     _                   _   |_|
    (_)_ __  _ __  _   _| |_
    | | `_ \| `_ \| | | | __|
    | | | | | |_) | |_| | |_
    |_|_| |_| .__/ \__,_|\__|
            |_|
    */
    %utlfkil(d:/htm/googlinecht.html);

    libname sd1 "d:/sd1";

    proc sql;
      create
         table sd1.cars as
      select
         type          as country
        ,avg(mpg_city) as avgmpg
      from
         sashelp.cars
      group
         by type
    ;quit;

    /*
     _ __  _ __ ___   ___ ___  ___ ___
    | `_ \| `__/ _ \ / __/ _ \/ __/ __|
    | |_) | | | (_) | (_|  __/\__ \__ \
    | .__/|_|  \___/ \___\___||___/___/
    |_|
    */
    %utlfkil(d:/htm/linecht.html);

    %utl_submit_r64('
    library(haven);
    library(googleVis);
    df<-read_sas("d:/sd1/cars.sas7bdat");
    str(df);
    line <- gvisLineChart(
        df
        ,options=list(width=500, height=300));
    print(line, file="d:/htm/googlinecht.html");
    ');

    /**************************************************************************************************************************/
    /*                                                                                                                        */
    /*                                                                                                                        */
    /*                              Car Type                                                                                  */
    /*                                                                                                                        */
    /*             Hybrid     SUV     Sedan   Sports   Truck   Wagon                                                          */
    /*             ---+--------+--------+--------+-------+--------+---                                                        */
    /*             |                                                 |                                                        */
    /*          60 +  X                                              +  60                                                    */
    /*             |   \         MPG by Car Type                     |                                                        */
    /*             |    \                                            |                                                        */
    /*             |     \                                           |     M                                                  */
    /*       M  40 +      \                                          +  40 P                                                  */
    /*       P     |       \                                         |     G                                                  */
    /*       G     |        \         / X                            |                                                        */
    /*             |         \      /    \                    ._-X   |                                                        */
    /*          20 +          \   /       \             /X  ./       +  20                                                    */
    /*             |           X/          \           /  \/      \  |                                                        */
    /*             |                        \        /               |                                                        */
    /*             |                         \     /                 |                                                        */
    /*           0 +                          \   /                  +   0                                                    */
    /*             |                            \X                   |                                                        */
    /*             ---+--------+--------+--------+-------+-------+----                                                        */
    /*             Hybrid     SUV     Sedan   Sports   Truck   Wagon                                                          */
    /*                                                                                                                        */
    /*                             Car Type                                                                                   */
    /*                                                                                                                        */
    /**************************************************************************************************************************/


    /*  _     _     _             _                     _ _
    | || |   | |__ | | __ _ _ __ | | __   __ _ ___  ___(_|_)  _   _ ___   _ __ ___   __ _ _ __
    | || |_  | `_ \| |/ _` | `_ \| |/ /  / _` / __|/ __| | | | | | / __| | `_ ` _ \ / _` | `_ \
    |__   _| | |_) | | (_| | | | |   <  | (_| \__ \ (__| | | | |_| \__ \ | | | | | | (_| | |_) |
       |_|   |_.__/|_|\__,_|_| |_|_|\_\  \__,_|___/\___|_|_|  \__,_|___/ |_| |_| |_|\__,_| .__/
                                                                                         |_|
    */
                                                         _
    ________________________                            / \
    |     ||         |      |------ _  _           ____(  _)
    |     |(         |      |    /_/ \// _        / | || /
    |_____| \        |------|   /  --\/ / \    __/  | /|(
    |     |  \_------|      |   \_   | (  (   /     |/_|/
    |     |    |     |______|_____\ _| |   \_/ ___ _|_|/
    (_____|____|     |       \     \ \_)------/   ) )
    |   |    | |_____|__      )____/  |  |    |___)/
    (   |    |    |     |_____|    \  |  /---/\__  \
    )   |    |    |     |      \    \ )_/    \ / \_/
    \   \    |    |     |       |    ||______//____|
     |   \   |____|_____|_______|----|       /      \
     )    \  /    |    |___     |    /______/_____  /
     \     \|     |    |  |     |   |   |   / \   \/
      \     \     |    |  |__   |---/   |   \  \  /
       \_   /     |    |     ---)   )   |   |   \/
         \__\_____|_----        \   |-|_|_---___/
                     \           )__  |   \/\_  \
                      \_/\      /   --        )  \
                          \    /              \  (
                           \  (                |  |
                            \__\                \_|

    /*              _
      ___ _ __   __| |
     / _ \ `_ \ / _` |
    |  __/ | | | (_| |
     \___|_| |_|\__,_|

    */
